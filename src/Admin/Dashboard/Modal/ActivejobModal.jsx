...
const ActivejobModal = ({ open, onClose }) => {

    const { showAlert } = useAlert();

    const handleConfirm = async () => {
        try {
            // Use the selectedJob.jobId and selectedJob.jobStatus here to update the status
            const response = await fetch(`http://localhost:3001/api/admin/job/updateStatus/${selectedJob.jobId}`, {
                method: 'PUT',
            });
            ...
            if (response.ok) {
                ...
                showAlert('error', data.error || 'Failed to update job status.');
            }
        } catch (error) {
        ...
        <Dialog open={open} handler={onClose} className="fixed inset-0 flex items-center justify-center backdrop-blur-md bg-opacity-5 text-gray-100 dark:text-gray-900">
            <div className="max-w-xl bg-gray-700 dark:bg-gray-100 p-8 rounded-md overflow-y-auto max-h-screen">
                ...
                <DialogBody className='text-gray-100 dark:text-gray-900'>
                    Are you sure you want to Active this?
                </DialogBody>
                <DialogFooter>
